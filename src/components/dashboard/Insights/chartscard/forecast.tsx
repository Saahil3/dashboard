
const Card = () => {
  return (
    
      <div className="absolute right-16 font-inter w-[260px] bg-gradient-to-br from-[#15B79F] to-[#0E9382] p-6 rounded-[20px] shadow-md">
        {/* Exactly 3 distinct circles */}
        <div
          className="absolute inset-0 opacity-70 rounded-[20px]"
          style={{
            background: `
              radial-gradient(
                circle at 15% 12%,
                #14B69E 0%,
                #14B69E 20%,
                #31BDA8 20%,
                #31BDA8 21%,
                #31BDA8 21%,
                #31BDA8 40%,
                #31BDA8 40%,
                #31BDA8 41%,
                #31B09F 41%,
                #31B09F 65%,
                #31B09F 65%,
                #31B09F 66%,
                #0F9785 66%,
                #0F9785 100%

                
                
              )
            `,
          }}
        />
        <div className="relative z-10">
          <div className="flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="18"
              height="20"
              fill="currentColor"
              className="mt-0.5 mr-1 text-gray-500"
              viewBox="0 0 18 16"
            >
              <path d="M9.875 4.8125C9.87364 6.05528 9.37934 7.24677 8.50056 8.12556C7.62177 9.00434 6.43028 9.49864 5.1875 9.5H1.23453C1.0398 9.49975 0.853111 9.42228 0.715413 9.28459C0.577716 9.14689 0.500248 8.9602 0.5 8.76547V4.8125C0.5 3.5693 0.99386 2.37701 1.87294 1.49794C2.75201 0.61886 3.9443 0.125 5.1875 0.125C6.4307 0.125 7.62299 0.61886 8.50206 1.49794C9.38114 2.37701 9.875 3.5693 9.875 4.8125Z" />
            </svg>
            <h3 className="text-xs text-[#CCFBEF] uppercase tracking-wider font-semibold">
              Forecasts
            </h3>
          </div>
          <div className="space-y-8 text-white">
            <div className="mt-6">
              <div className="flex items-center gap-6">
                <h1 className="text-6xl font-medium">+15%</h1>
                <svg
                  width="35"
                  height="35"
                  viewBox="0 0 35 35"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M32.8125 7.65625V16.4062C32.8125 16.6963 32.6973 16.9745 32.4921 17.1796C32.287 17.3848 32.0088 17.5 31.7187 17.5C31.4287 17.5 31.1505 17.3848 30.9454 17.1796C30.7402 16.9745 30.625 16.6963 30.625 16.4062V10.2963L19.3676 21.5551C19.266 21.6568 19.1454 21.7374 19.0126 21.7925C18.8798 21.8475 18.7375 21.8759 18.5937 21.8759C18.45 21.8759 18.3077 21.8475 18.1749 21.7925C18.0421 21.7374 17.9215 21.6568 17.8199 21.5551L13.125 16.8588L4.05508 25.9301C3.84984 26.1353 3.57149 26.2506 3.28125 26.2506C2.99101 26.2506 2.71265 26.1353 2.50742 25.9301C2.30219 25.7248 2.18689 25.4465 2.18689 25.1562C2.18689 24.866 2.30219 24.5877 2.50742 24.3824L12.3512 14.5387C12.4527 14.437 12.5734 14.3563 12.7062 14.3013C12.8389 14.2462 12.9813 14.2179 13.125 14.2179C13.2687 14.2179 13.4111 14.2462 13.5438 14.3013C13.6766 14.3563 13.7972 14.437 13.8988 14.5387L18.5937 19.235L29.0787 8.75H22.9687C22.6787 8.75 22.4005 8.63477 22.1954 8.42965C21.9902 8.22453 21.875 7.94633 21.875 7.65625C21.875 7.36617 21.9902 7.08797 22.1954 6.88285C22.4005 6.67773 22.6787 6.5625 22.9687 6.5625H31.7187C32.0088 6.5625 32.287 6.67773 32.4921 6.88285C32.6973 7.08797 32.8125 7.36617 32.8125 7.65625Z"
                    fill="#F0FDFA"
                  />
                </svg>
              </div>
              <p className="text-sm mt-4">
                forecasted increase in your sales closed by the end of the
                current month
              </p>
            </div>
            <div className="pt-1 pb-24">
              <div className="flex items-center gap-6">
                <h1 className="text-6xl font-medium">+20%</h1>
                <svg
                  width="35"
                  height="35"
                  viewBox="0 0 35 35"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M32.8125 7.65625V16.4062C32.8125 16.6963 32.6973 16.9745 32.4921 17.1796C32.287 17.3848 32.0088 17.5 31.7187 17.5C31.4287 17.5 31.1505 17.3848 30.9454 17.1796C30.7402 16.9745 30.625 16.6963 30.625 16.4062V10.2963L19.3676 21.5551C19.266 21.6568 19.1454 21.7374 19.0126 21.7925C18.8798 21.8475 18.7375 21.8759 18.5937 21.8759C18.45 21.8759 18.3077 21.8475 18.1749 21.7925C18.0421 21.7374 17.9215 21.6568 17.8199 21.5551L13.125 16.8588L4.05508 25.9301C3.84984 26.1353 3.57149 26.2506 3.28125 26.2506C2.99101 26.2506 2.71265 26.1353 2.50742 25.9301C2.30219 25.7248 2.18689 25.4465 2.18689 25.1562C2.18689 24.866 2.30219 24.5877 2.50742 24.3824L12.3512 14.5387C12.4527 14.437 12.5734 14.3563 12.7062 14.3013C12.8389 14.2462 12.9813 14.2179 13.125 14.2179C13.2687 14.2179 13.4111 14.2462 13.5438 14.3013C13.6766 14.3563 13.7972 14.437 13.8988 14.5387L18.5937 19.235L29.0787 8.75H22.9687C22.6787 8.75 22.4005 8.63477 22.1954 8.42965C21.9902 8.22453 21.875 7.94633 21.875 7.65625C21.875 7.36617 21.9902 7.08797 22.1954 6.88285C22.4005 6.67773 22.6787 6.5625 22.9687 6.5625H31.7187C32.0088 6.5625 32.287 6.67773 32.4921 6.88285C32.6973 7.08797 32.8125 7.36617 32.8125 7.65625Z"
                    fill="#F0FDFA"
                  />
                </svg>
              </div>
              <p className="text-sm mt-4">
                forecasted increase in consultations by the end of the current
                month
              </p>
            </div>
          </div>
        </div>
      </div>
    
  );
};

export default Card;
